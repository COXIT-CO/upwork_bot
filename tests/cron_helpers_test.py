...
os.environ['TESTING'] = "true"   # TESTING valiable should be initialized regardless the assigned value
from app.app import flask_app
from app.database import DB
from cron_jobs.helpers import remove_job_from_db, remove_unactive_jobs_from_db, find_new_job_openings
...
def test_remove_job_from_db(remove_job_from_db_setup, test):
    with flask_app.app_context():
        match test:
            case None:
                assert remove_job_from_db(None) == "You haven't provided job url! It's required"
            case "https://wrong_link":
                assert remove_job_from_db("https://wrong_link") == "No such job opening!"
            case "https://job_link.com":
                print(job_controller.get("https://job_link.com"))
                assert remove_job_from_db("https://job_link.com") == "Deleted successfully!"
...
def test_remove_unactive_jobs_from_db_setup():
    with flask_app.app_context():
        for i in range(5):
            job_controller.create(job_url=f"https://upwork_link_{i}.com", origin="upwork")
        for i in range(5):
            job_controller.create(job_url=f"https://linkedin_link_{i}.com", origin="linkedin")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (3, [f"https://upwork_link_{i}.com" for i in range(3)] + ["https://unexisting_link.com" for _ in range(3)])    # active jobs
    ],
)
def test_remove_unactive_jobs_from_db(test_remove_unactive_jobs_from_db_setup, id, test):
    with flask_app.app_context():
        match id:
            case 1:
            ...
            case 3:
                remove_unactive_jobs_from_db(test, "upwork")
                with flask_app.app_context():
                    for job_url in [f"https://upwork_link_{i}.com" for i in range(3, 5)]:
                        if job_controller.get(job_url) != "":
                            pytest.fail(f"Job {job_url} must be removed!")
                    for job_url in [f"https://linkedin_link_{i}.com" for i in range(5)]:
                        if job_controller.get(job_url) == "":
...
@pytest.mark.parametrize(
    "id,test",
    [
        (1, [{"job_title": "upwork job {i}", "job_url": f"https://upwork_link_{i}.com"} for i in range(5, 7)]),   # all new jobs
        (2, [{"job_title": "upwork job {i}", "job_url": f"https://upwork_link_{i}.com"} for i in range(5)]),      # already existing jobs
        (3, [{"job_title": "upwork job {i}", "job_url": f"https://upwork_link_{i}.com"} for i in range(3, 7)]),   # partially new jobs
        (4, [{"job_title": "linkedin job {i}", "job_url": f"https://linkedin_link_{i}.com"} for i in range(2)]),  # all new jobs with other origin
    ],
)
